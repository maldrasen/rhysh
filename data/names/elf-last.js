let names = [
  { name:'Amberleaf' },
  { name:'Ambersky' },
  { name:'Araventer' },
  { name:'Arrowswift' },
  { name:'Ashenvale' },
  { name:'Autumnlight' },
  { name:'Barrows' },
  { name:'Bearmantle', triggers:['strong']},
  { name:'Berry' },
  { name:'Blackbear' },
  { name:'Blackleaf' },
  { name:'Blackriver' },
  { name:'Blackwolf' },
  { name:'Blight' },
  { name:'Bloomthistle' },
  { name:'Bonechewer', aspects:['violent.3']},
  { name:'Bramblethorn' },
  { name:'Brier' },
  { name:'Brierborn' },
  { name:'Briers' },
  { name:'Brightfang', aspects:['violent.2']},
  { name:'Brightmoon' },
  { name:'Brightwood' },
  { name:'Cliffton' },
  { name:'Cloudpath' },
  { name:'Cross' },
  { name:'Crown' },
  { name:'Darkhollow' },
  { name:'Darksky' },
  { name:'Deeper' },
  { name:'Dewshine' },
  { name:'Duskgrove' },
  { name:'Farrowdeep' },
  { name:'Feathermoon' },
  { name:'Fielder' },
  { name:'Fields' },
  { name:'Fox' },
  { name:'Gatekeeper' },
  { name:'Gatewatcher' },
  { name:'Goldshadow' },
  { name:'Goodberry' },
  { name:'Greenleaf' },
  { name:'Greenwood' },
  { name:'Greywarden' },
  { name:'Greywatcher' },
  { name:'Grimclaw', aspects:['violent.2']},
  { name:'Grimfang', aspects:['violent.2']},
  { name:'Grovetender' },
  { name:'Heartwood' },
  { name:'Highfisher' },
  { name:'Highlander' },
  { name:'Highranger' },
  { name:'Howling' },
  { name:'Irons' },
  { name:'Irontowner' },
  { name:'Lightleaf' },
  { name:'Mapleheart' },
  { name:'Marrowwhite' },
  { name:'Mistdancer' },
  { name:'Mistwarden' },
  { name:'Moonbreeze' },
  { name:'Moonfeather' },
  { name:'Moonrage' },
  { name:'Moonsong' },
  { name:'Moss' },
  { name:'Mossglow', triggers:['magical']},
  { name:'Nettlepatch' },
  { name:'Nightsong' },
  { name:'Nightwhisper' },
  { name:'Nightwing' },
  { name:'Nightwish' },
  { name:'Northfield' },
  { name:'Oakenbough' },
  { name:'Oakheart' },
  { name:'Olendalier' },
  { name:'Raindancer' },
  { name:'Ravenclaw' },
  { name:'Raveneye' },
  { name:'Redleaf' },
  { name:'Redwood' },
  { name:'Rockstalker' },
  { name:'Rushwater' },
  { name:'Sharptalon' },
  { name:'Silver' },
  { name:'Silverlight' },
  { name:'Silverman', restriction:_male},
  { name:'Silveroak' },
  { name:'Silverpine' },
  { name:'Silverton' },
  { name:'Skullkin' },
  { name:'Skyclad', triggers:['beautiful']},
  { name:'Skydancer' },
  { name:'Skymane' },
  { name:'Skyshadow' },
  { name:'Skysong' },
  { name:'Skywatcher' },
  { name:'Southfield' },
  { name:'Springlight' },
  { name:'Springsong' },
  { name:'Stagheart' },
  { name:'Staghelm' },
  { name:'Starbreeze' },
  { name:'Starseeker' },
  { name:'Stoneman', restriction:_male},
  { name:'Stonetalon' },
  { name:'Stonetower' },
  { name:'Stonetowner' },
  { name:'Stonetree' },
  { name:'Stormrage', triggers:['magical']},
  { name:'Swiftriver' },
  { name:'Tailchaser', aspects:['slut.2']},
  { name:'Thistlewood' },
  { name:'Thirdleg', triggers:['monster-cock'], restriction:_notFemale},
  { name:'Whitewater' },
  { name:'Willowfall' },
  { name:'Windsinger' },
  { name:'Wolfrider', triggers:['strong']},
  { name:'Wolfrunner' },
  { name:"the Drafthorse", triggers:['tall','strong','monster-cock','horse-cock','big-balls'], restriction:_male},
  { name:"the Fool", triggers:['stupid'], restriction:_male},
];

names.forEach(name => {
  NameBuilder.addName(name, 'ElfLast');
});
